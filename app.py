...
app.layout=html.Div([
    ...
    html.Div([
        ...
        html.Div([
            html.Div([
                html.Div([
                    dcc.Dropdown(id='my-dropdown',options=options2,value='Status'),
...
def generate_graph(type):
    if type=='All':
        return {
            'data':[go.Line(x=patients['Status'],y=patients['Total'])],
...
@app.callback(Output('the-graph','figure'),[Input('my-dropdown','value')])
def generate_graph(my_dropdown):
    return px.pie(data_frame=patients,names=my_dropdown,hole=0.3)
    #return (piechart)
